 ...
 function mitta__create_mitta(mode, type, mode_name, mode_ycord, mode_xcord, mode_hcord, mode_wcord, mode_count, mode_id, mode_specifications,ir_mitta_comment,ir_mitta_cfrom,ir_mitta_cto) {
   ...
   if (mode) {
     ...
     if (input_step == "drawscreen_section_two") {
       ...
       newDiv__comment_settings.setAttribute("onclick",
         "obj = this.getAttribute('name');document.querySelector('#drawscreen_section_two > div.modal-container').classList.add('two');document.querySelector('#drawscreen_section_two > div.modal-container').classList.remove('out');document.querySelector('body').classList.add('modal-active');settings__mitta();change__newdiv_cord();submitprogress('','cancel','" +
         id + "','aukot','" + t_array + "');au_previous_vord = '" + au_previous_vord + "';aukko_del(document.querySelector('#'+obj), -1);");
